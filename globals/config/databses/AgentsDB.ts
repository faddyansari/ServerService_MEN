...
let dbName = (process.env.NODE_ENV == 'production') ? 'agentsDB' : "agentsDB";
// let dbName = 'local';
...
export class AgentsDB {
    ...
    private static Instance: AgentsDB;
    ...
    public static async connect(prodURI?: string): Promise<Db> {
        ...
        try {
            if (!AgentsDB.Instance) {
                AgentsDB.Instance = new AgentsDB();
                let mongoClient = await AgentsDB.Instance.connectDatabase(prodURI);
                ...
                return AgentsDB.Instance.dataBase;
            } else return AgentsDB.Instance.dataBase
    ...
    public async connectDatabase(prodURI) {
        try {
            ...
            let client = await AgentsDB.Instance.connectDatabase(prodURI)
            return client;
        }
    }
