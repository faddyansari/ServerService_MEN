...
let dbName = (process.env.NODE_ENV == 'production') ? 'chatsDB' : "chatsDB";
...
export class ChatsDB {
    ...
    private static Instance: ChatsDB;
    ...
    public static async connect(prodURI?: string): Promise<Db> {
        ...
        try {
            if (!ChatsDB.Instance) {
                ...
                let mongoClient = await ChatsDB.Instance.connectDatabase(prodURI);
                ...
                return ChatsDB.Instance.dataBase;
            } else return ChatsDB.Instance.dataBase
    ...
    public async connectDatabase(prodURI) {
        try {
            ...
            let client = await ChatsDB.Instance.connectDatabase(prodURI)
            return client;
        }
    }
