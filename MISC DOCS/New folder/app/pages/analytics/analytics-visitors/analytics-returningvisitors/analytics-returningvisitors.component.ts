...
export class AnalyticsReturningvisitorsComponent implements OnInit {
	...
	constructor(_authService: AuthService, public _analyticsService: AnalyticsService) {
		...
		this.subscriptions.push(_analyticsService.options.subscribe(data => {
			if (this.rendered) {
				if (data.title.text) {
					this.options.title.text = 'Returning Visitors (' + data.title.text + ')'
				}else{
					this.options.title.text = 'Returning Visitors';
				}
	...
	onFilterResult(event){
		this._analyticsService.GetReturningVisitors(event.csid,event.selectedDateType, event.selectedDate).subscribe(data => {
			// let visitorIDs = [];
			...
			if (event.comparison) {
				...
				if (event.selectedComparison.match(/days/g)) {
					...
					data.json().forEach(element => {
						...
						if (daysData.filter(t => t.name == (this._analyticsService.dayNames[new Date(day).getDay()] + " " + new Date(day).getDate())).length) {
							daysData.filter(t => t.name == (this._analyticsService.dayNames[new Date(day).getDay()] + " " + new Date(day).getDate()))[0].value += element.returningVisitors;
						}
					});
					if (!data.json().length) {
						this._analyticsService.updateChart('line', daysData, 'flat', 'Returning Visitors', true);
					} else {
						this._analyticsService.updateChart('line', daysData, 'flat', 'Returning Visitors', false, returningVisitors);
					}
					...
					data.json().forEach(element => {
						...
						if (temp_graph.filter(t => t.name == month).length) {
							temp_graph.filter(t => t.name == month)[0].value += element.returningVisitors;
						}
					});
					if (!data.json().length) {
						this._analyticsService.updateChart('line', temp_graph, 'flat', 'Returning Visitors', true);
					} else {
						this._analyticsService.updateChart('line', temp_graph, 'flat', 'Returning Visitors', false, returningVisitors);
					}
					...
					data.json().forEach(element => {
						...
						if (yearsData.filter(t => t.name == year).length) {
							yearsData.filter(t => t.name == year)[0].value += element.returningVisitors;
						}
					});
					...
					if (!data.json().length) {
						...
						this._analyticsService.updateChart('column', yearsData, 'flat', 'Total Visitors', false, returningVisitors);
					}
				}
			} else {
				switch (event.selectedDateType) {
					...
					case 'yesterday':
						...
						if (!data.json().length) {
							this._analyticsService.updateChart('line', firstHalf.concat(secondHalf), 'flat', 'Returning Visitors', true);
							// return;
						} else {
							this._analyticsService.updateChart('line', firstHalf.concat(secondHalf), 'flat', 'Returning Visitors', false, returningVisitors);
						}
						break;
					case 'week':
						let temp_graph = [];
					...
					case 'month':
						...
						data.json().forEach(element => {
							...
							if (graph_month.filter(t => t.name == month).length) {
								graph_month.filter(t => t.name == month)[0].value += element.returningVisitors;
							}
						});
						// this.graph_uniqueVisitors = graph_month;
						if (!data.json().length) {
							this._analyticsService.updateChart('line', graph_month, 'flat', 'Returning Visitors', true);
							// return;
						} else {
							this._analyticsService.updateChart('line', graph_month, 'flat', 'Returning Visitors', false, returningVisitors);
						}
						break;
					default:
						...
						if (this._analyticsService.daysBetween(date1, date2) == 0 || this._analyticsService.daysBetween(date1, date2) == 1) {
							...
							data.json().forEach(element => {
								...
								if (graph_week.filter(t => t.name == month).length) {
									graph_week.filter(t => t.name == month)[0].value += element.returningVisitors;
								}
							});
							// this.graph_uniqueVisitors = graph_week;
							if (!data.json().length) {
								this._analyticsService.updateChart('line', graph_week, 'flat', 'Returning Visitors', true);
								// return;
							} else {
								this._analyticsService.updateChart('line', graph_week, 'flat', 'Returning Visitors', false, returningVisitors);
							}
						} else {
							let temp_graph = [];
							for (let i = this._analyticsService.daysBetween(date1, date2); i >= 0; i--) {
