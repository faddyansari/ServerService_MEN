...
export class AnalyticsRegvialivechatComponent implements OnInit {
  ...
  onFilterResult(event) {
			...
			this._analyticsService.GetTotalRegViaLivechat(packet).subscribe(response => {	
        if(response['Total Reg via LiveChat'] && response['Total Reg via LiveChat'].length){
					this.regViaLiveChatData = response['Total Reg via LiveChat'];
	...
	populateGraph24Hour() {
		...
		this.regViaLiveChatData.forEach(element => {
			series.push({
				name: element.name,
				data: element.data
			})
