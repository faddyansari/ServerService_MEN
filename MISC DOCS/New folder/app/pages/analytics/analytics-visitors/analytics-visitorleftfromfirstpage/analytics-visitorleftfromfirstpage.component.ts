...
export class AnalyticsVisitorleftfromfirstpageComponent implements OnInit {
  ...
  subscriptions: Subscription[] = [];
	...
	visitorsLeftFromFirstPageData: any = [];
  country: any;
  agent : any;
	options: any;
  ...
  onFilterResult(event) {
			...
			this._analyticsService.GetTotalVisitorsLeftFromFirstPage(packet).subscribe(response => {	
        if(response['Visitors Left From First Page'] && response['Visitors Left From First Page'].length){
					this.visitorsLeftFromFirstPageData = response['Visitors Left From First Page'];
...
this.loading = false;
	...
	populateGraph24Hour() {
		...
		this.visitorsLeftFromFirstPageData.forEach(element => {
			series.push({
				name: element.name,
				data: element.data
			})
