...
export class AnalyticsAgentfeedbackComponent implements OnInit {
	...
	onFilterResult(event){
		this._analyticsService.GetAgentFeedback(event.csid,event.selectedDateType, event.selectedDate, this.selectedAgents).subscribe(data => {
			...
			if (event.comparison) {
				...
				if (event.selectedComparison.match(/days/g)) {
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('line', series_group, 'dimensional', 'Feedbacks', true);
						// return;
					} else {
						this._analyticsService.updateChart('line', series_group, 'dimensional', 'Feedbacks');
					}
				...
				if (!this.selectedAgents.length) {
					switch (event.selectedDateType) {
						...
						case 'yesterday':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('line', graph_temp, 'dimensional', 'Feedbacks', true);
								// return;
							} else {
								this._analyticsService.updateChart('line', graph_temp, 'dimensional', 'Feedbacks');
							}
							break;
						case 'week':
							let series_group = [
						...
						case 'month':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('line', graph_month, 'dimensional', 'Feedbacks', true);
								// return;
							} else {
								this._analyticsService.updateChart('line', graph_month, 'dimensional', 'Feedbacks');
							}
							break;
						default:
