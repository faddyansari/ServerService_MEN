...
export class AnalyticsChatdurationComponent implements OnInit {
	...
	constructor(_authService: AuthService, public _analyticsService: AnalyticsService) {
		...
		this.subscriptions.push(_analyticsService.options.subscribe(data => {
			if(this.rendered){
				if(data.title.text) {
					this.options.title.text = 'Chat Duration (' + this.time_convert(data.title.text) + ')'
				}else{
					this.options.title.text = 'Chat Duration'
				}
	...
	onFilterResult(event){
		this._analyticsService.GetChatDuration(event.csid, event.selectedDateType,event.selectedDate, event.selectedAgents).subscribe(data => {
			...
			if (event.comparison) {
				...
				if (event.selectedComparison.match(/days/g)) {
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', daysData, 'dimensional', 'Duration', true);
					} else {
						this._analyticsService.updateChart('column', daysData, 'dimensional', 'Duration', false, totalDuration);
					}
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', monthsData, 'flat', 'Duration', true);
					} else {
						totalDuration = totalDuration / data.json().length;
						this._analyticsService.updateChart('column', monthsData, 'flat', 'Duration', false, totalDuration);
					}
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', yearsData, 'dimensional', 'Duration', true);
					} else {
						totalDuration = totalDuration / data.json().length;
						this._analyticsService.updateChart('column', yearsData, 'dimensional', 'Duration', false, totalDuration);
					}
				}
			} else {
				if (!event.selectedAgents.length) {
					switch (event.selectedDateType) {
						...
						case 'yesterday':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', firstHalf.concat(secondHalf), 'flat', 'Duration', true);
								// return;
							} else {
								totalDuration = totalDuration / data.json().length;
								this._analyticsService.updateChart('column', firstHalf.concat(secondHalf), 'flat', 'Duration', false, totalDuration);
							}
							break;
						case 'week':
							let graph_week = [];
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_week, 'flat', 'Duration', true);
								// return;
							} else {
								totalDuration = totalDuration / data.json().length;
								this._analyticsService.updateChart('column', graph_week, 'flat', 'Duration', false, totalDuration);
							}
							break;
						case 'month':
							let graph_month = [];
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_month, 'flat', 'Duration', true);
								// return;
							} else {
								totalDuration = totalDuration / data.json().length;
								this._analyticsService.updateChart('column', graph_month, 'flat', 'Duration', false, totalDuration);
							}
							break;
						default:
					...
					switch (event.selectedDateType) {
						...
						case 'yesterday':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_agents, 'flat', 'Duration', true);
								// return;
							} else {
								totalDuration = totalDuration / data.json().length;
								this._analyticsService.updateChart('column', graph_agents, 'agents', 'Duration', false, totalDuration);
							}
							break;
						case 'week':
							event.selectedAgents.forEach(agent => {
