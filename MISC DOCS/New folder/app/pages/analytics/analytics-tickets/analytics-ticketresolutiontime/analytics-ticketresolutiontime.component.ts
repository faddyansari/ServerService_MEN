...
export class AnalyticsTicketresolutiontimeComponent implements OnInit {
	...
	constructor(_authService: AuthService, public _analyticsService: AnalyticsService) {
		...
		this.subscriptions.push(_analyticsService.options.subscribe(data => {
			if(this.rendered){
				if(data.title.text) {
					this.options.title.text = 'Ticket Resolution Time (' + this.time_convert(data.title.text) + ')'
				}
	...
	onFilterResult(event){
		this._analyticsService.GetTicketResolutionTime(event.csid, event.selectedDateType,event.selectedDate, event.selectedAgents).subscribe(data => {
			...
			if (event.comparison) {
				...
				if (event.selectedComparison.match(/days/g)) {
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', daysData, 'dimensional', 'Resolution Time', true);
					} else {
						this._analyticsService.updateChart('column', daysData, 'dimensional', 'Resolution Time', false, totalResolutionTime);
					}
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', monthsData, 'flat', 'Resolution Time', true);
					} else {
						this._analyticsService.updateChart('column', monthsData, 'flat', 'Resolution Time', false, totalResolutionTime);
					}
					...
					if (!data.json().length) {
						this._analyticsService.updateChart('column', yearsData, 'dimensional', 'Resolution Time', true);
					} else {
						this._analyticsService.updateChart('column', yearsData, 'dimensional', 'Resolution Time', false, totalResolutionTime);
					}
				}
			} else {
				if (!event.selectedAgents.length) {
					switch (event.selectedDateType) {
						...
						case 'yesterday':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', firstHalf.concat(secondHalf), 'flat', 'Resolution Time', true);
								// return;
							} else {
								this._analyticsService.updateChart('column', firstHalf.concat(secondHalf), 'flat', 'Resolution Time', false, totalResolutionTime);
							}
							break;
						case 'week':
							let graph_week = [];
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_week, 'flat', 'Resolution Time', true);
								// return;
							} else {
								this._analyticsService.updateChart('column', graph_week, 'flat', 'Resolution Time', false, totalResolutionTime);
							}
							break;
						case 'month':
							let graph_month = [];
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_month, 'flat', 'Resolution Time', true);
								// return;
							} else {
								this._analyticsService.updateChart('column', graph_month, 'flat', 'Resolution Time', false, totalResolutionTime);
							}
							break;
						default:
					...
					switch (event.selectedDateType) {
						...
						case 'yesterday':
							...
							if (!data.json().length) {
								this._analyticsService.updateChart('column', graph_agents, 'flat', 'Resolution Time', true);
								// return;
							} else {
								this._analyticsService.updateChart('column', graph_agents, 'agents', 'Resolution Time', false, totalResolutionTime);
							}
							break;
						case 'week':
							event.selectedAgents.forEach(agent => {
