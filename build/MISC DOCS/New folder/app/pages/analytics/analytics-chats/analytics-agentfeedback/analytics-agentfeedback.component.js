...
exports.AnalyticsAgentfeedbackComponent = void 0;
var core_1 = require("@angular/core");
var Highcharts = require("highcharts");
require('highcharts/modules/exporting')(Highcharts);
require('highcharts/modules/export-data')(Highcharts);
var AnalyticsAgentfeedbackComponent = /** @class */ (function () {
    function AnalyticsAgentfeedbackComponent(_authService, _chatService, _globalStateService, _analyticsService) {
        var _this = this;
    ...
    AnalyticsAgentfeedbackComponent.prototype.ngOnInit = function () {
    };
    AnalyticsAgentfeedbackComponent.prototype.onFilterResult = function (event) {
        var _this = this;
        this._analyticsService.GetAgentFeedback(event.csid, event.selectedDateType, event.selectedDate, this.selectedAgents).subscribe(function (data) {
            ...
            if (event.comparison) {
                ...
                if (event.selectedComparison.match(/days/g)) {
                    ...
                    if (!data.json().length) {
                        _this._analyticsService.updateChart('line', series_group, 'dimensional', 'Feedbacks', true);
                        // return;
                    }
                    else {
                        _this._analyticsService.updateChart('line', series_group, 'dimensional', 'Feedbacks');
                    }
                ...
                if (!_this.selectedAgents.length) {
                    switch (event.selectedDateType) {
                        ...
                        case 'yesterday':
                            ...
                            if (!data.json().length) {
                                _this._analyticsService.updateChart('line', graph_temp, 'dimensional', 'Feedbacks', true);
                                // return;
                            }
                            else {
                                _this._analyticsService.updateChart('line', graph_temp, 'dimensional', 'Feedbacks');
                            }
                            break;
                        case 'week':
                            var series_group = [
                        ...
                        case 'month':
                            ...
                            if (!data.json().length) {
                                _this._analyticsService.updateChart('line', graph_month, 'dimensional', 'Feedbacks', true);
                                // return;
                            }
                            else {
                                _this._analyticsService.updateChart('line', graph_month, 'dimensional', 'Feedbacks');
                            }
                            break;
                        default:
                            ...
                            if (_this._analyticsService.daysBetween(date1, date2) == 0 || _this._analyticsService.daysBetween(date1, date2) == 1) {
                                ...
                                if (!data.json().length) {
                                    _this._analyticsService.updateChart('line', graph_temp_1, 'dimensional', 'Feedbacks', true);
                                    // return;
                                }
                                else {
                                    _this._analyticsService.updateChart('line', graph_temp_1, 'dimensional', 'Feedbacks');
                                }
                            }
                                ...
                                if (!data.json().length) {
                                    _this._analyticsService.updateChart('line', series_group_1, 'dimensional', 'Feedbacks', true);
                                    // return;
                                }
                                else {
                                    _this._analyticsService.updateChart('line', series_group_1, 'dimensional', 'Feedbacks');
                                }
                            }
                            else {
                                var graph_month_1 = [
                                ...
                                if (!data.json().length) {
                                    _this._analyticsService.updateChart('line', graph_month_1, 'dimensional', 'Feedbacks', true);
                                    // return;
                                }
                                else {
                                    _this._analyticsService.updateChart('line', graph_month_1, 'dimensional', 'Feedbacks');
                                }
                            }
                            break;
                    }
                    ...
                    data.json().forEach(function (element, index) {
                        ...
                        _this.selectedAgents.forEach(function (agent) {
                            ...
                            obj[element.date].good.push({
                                email: agent, count: (element.email == agent) ? element.good : 0
                            });
                            obj[element.date].average.push({
                                email: agent, count: (element.email == agent) ? element.average : 0
                            });
                            obj[element.date].bad.push({
                                email: agent, count: (element.email == agent) ? element.bad : 0
                            });
                        });
    ...
    AnalyticsAgentfeedbackComponent.prototype.ngAfterViewInit = function () {
        this.chart = new Highcharts.Chart("highchart", this.options);
        this.rendered = true;
    };
    AnalyticsAgentfeedbackComponent.prototype.showTotal = function (indexExists, index) {
        var count = 0;
        if (indexExists) {
    ...
    AnalyticsAgentfeedbackComponent.prototype.Export = function () {
        console.log('Exporting...');
        // console.log(this.agentTableData);
        this._analyticsService.exportHTMLToExcel('agentFeedback', 'ChatFeedback-' + new Date().getTime());
    };
    AnalyticsAgentfeedbackComponent.prototype.showPercentage = function () {
        var count = 0;
        var unfilled = 0;
        this.options.series.forEach(function (element) {
            count += element.data.reduce(function (a, b) { return a + b; }, 0);
    ...
    AnalyticsAgentfeedbackComponent.prototype.showSeriesCount = function (index) {
        var count = 0;
    ...
    AnalyticsAgentfeedbackComponent.prototype.getKeys = function (obj) {
        var keys = [];
    ...
    AnalyticsAgentfeedbackComponent.prototype.getValueAgainstKey = function (obj, date, key, agent) {
        return obj[date][key].filter(function (a) { return a.email == agent; })[0].count;
    };
    AnalyticsAgentfeedbackComponent.prototype.ngOnDestroy = function () {
        this.subscriptions.forEach(function (subscription) {
            subscription.unsubscribe();
        });
    };
    AnalyticsAgentfeedbackComponent = __decorate([
        core_1.Component({
            selector: 'app-analytics-agentfeedback',
            templateUrl: './analytics-agentfeedback.component.html',
            styleUrls: ['./analytics-agentfeedback.component.css'],
            encapsulation: core_1.ViewEncapsulation.None
        })
    ], AnalyticsAgentfeedbackComponent);
    return AnalyticsAgentfeedbackComponent;
}());
exports.AnalyticsAgentfeedbackComponent = AnalyticsAgentfeedbackComponent;
//# sourceMappingURL=analytics-agentfeedback.component.js.map